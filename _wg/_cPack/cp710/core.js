...
$('.panel_cp710').d_quickFloating({
    dir:"bottom",
    openState:1,            //오픈상태(1열림/0닫힘)
    closeGap:0,		        //닫힐때 우측여백
    motionSpeed:700,        //닫힘속도(0~)
    cookieDay:2,            //쿠키유지(일)
    autoCloseTime:30000      //자동닫힘시간(1초=1000)
});
...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    $('.contact_ok').addClass('disabled');
    $('.contact_ok').click(function(e) {
        var $btn = $(this);
    ...
    $('#confirm_contact').click(function(e) {
        var $cb = $(this);
        if($cb.attr('checked')) {
            $('.confirm').removeAttr('disabled');
            $('.contact_ok').removeClass('disabled');
        } else {
            $('.confirm').attr('disabled','disabled');
            $('.contact_ok').addClass('disabled');
        }

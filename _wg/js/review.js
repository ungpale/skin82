...
$(function(){
    ...
    $('.xans-product-review a').each(function(){
        ...
        var _this = $(this);
        var limit = 3;  // 이미지 수량
        var imgWidth = 300;  // 이미지 너비
        var imgHeight = 300;  // 이미지 높이
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (images.length > limit) { 
                    moreBtn = "<a href='#none' style='display:inline-block; font-size: 24px; margin: 0 12px;color:#1e1e1e; vertical-align:middle;' onclick='$(this).siblings(\"canvas\").show();$(this).hide();event.stopPropagation();'>+</a>" 
                }
                var sImg = $('<div class="review_image_wrap" style="display: inline-block;"></div>');
                ...
                images.each(function(i){
                    ...
                    img.onload = function() {
                        ...
                        if (imagesProcessed === images.length) {
                            ...
                            _this.parents('li').find('.img_content').append(sImg);
                        }
                    };
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.comment != undefined && data.read['comment_count'] > 0) {
                        aHtml.push('<div class="gLeft">');
                        aHtml.push('<a href="#none" class="btnNormal" onclick="REVIEW.comment_view('+data.read['no']+');">댓글보기 <em>('+data.read['comment_count']+')</em></a> <a href="#none" class="btnNormal" onclick="REVIEW.comment_write(this);"><span class="ico write"></span> 쓰기</a>');
                        aHtml.push('</div>');
                    }
                    if (data.write_auth == true) {
                        aHtml.push('<span class="gRight">');
                        aHtml.push('<a href="/board/product/modify.html?board_act=edit&no='+data.no+'&board_no=4&link_product_no='+iProductNo+'" class="btnNormal">수정</a>');
                        aHtml.push('</span>');
                    }
                    aHtml.push('</div>');
                        ...
                        for (var i=0; data.comment.length > i; i++) {
                            ...
                            if (data.comment[i]['comment_reply_css'] == undefined) {
                                ...
                                if (data.comment[i]['comment_reply_display'] == true) {
                                    ...
                                    aHtml.push('<a href="#none" class="btnNormal" onclick="REVIEW.comment_reply_view('+data.comment[i]['comment_no']+')">댓글의 댓글 <em>('+data.comment[i]['comment_reply_count']+')</em></a>');
                                    aHtml.push('<a href="#none" class="btnNormal" onclick="'+data.comment[i]['action_comment_reply_new']+'"><span class="ico write"></span> 쓰기</a>');
                                    aHtml.push('</div>'+'</div>');
                                }
                                aHtml.push('</li>');
                            } else {
                    ...
                    if (data.comment_write != undefined) {
                        ...
                        aHtml.push('<div class="submit"><a href="#none" onclick="' +data.comment_write['action_comment_insert']+ '" class="btnStrong">댓글 입력</a></div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                    ...
                    if (data.comment_reply != undefined) {
                        ...
                        aHtml.push('<div class="submit"><a href="#none" onclick="' +data.comment_reply['action_comment_insert']+ '" class="btnStrong">입력</a></div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
...
$(document).ready(function(){
    $(".review_content .review_summary").each(function() {
        var reviewSummary = $(this).html();
        var updatedSummary = reviewSummary.replace(/(<br\s*\/?>\s*){2,}/gi, '<br>');
        $(this).html(updatedSummary);
    });
})
...
!(function ($) {
  $(function () {
    if ($.cookie('ck_colorList') != 'hide' && $('.colorListBtn').css('display') != 'none') {
      //버튼이 보이지 않을 때 자동 팝업 띄우지 않는다.
      setTimeout(function () {
        $('.colorListBtn').trigger('click');
      }, 180000); //1000 = 1초후 열기
    }
    jQuery('.colorList.popup').on('click', '.p-close', function (e) {
      $.cookie('ck_colorList', 'hide', { path: '/', expires: 1 });
    }); //오늘하루 열지않음
    $('.colorListBtn').click(function (e) {
      var $btn = $(this);
      $btn.blur();
      $('.colorList.popup').bPopup(
        {
          zIndex: 999, // z-index
          positionStyle: 'fixed', // 고정방법
          speed: 300, // 속도 (css transition과 동일하게 설정)
          autoClose: 200000, // 1000 = 1초후 닫힘
          modal: false, // 배경
          transition: 'custom',
          follow: [false, false],
          openClass: 'on',
...
// 컬러 버튼
!(function($){
	$.fn.DB_tabAddClass=function(options){
		var opt={init:null};
		$.extend(opt,options);
		return this.each(function(){
			...
			var $mColor=$this.find('.mColor');
			var len=$tab_li.length;
			var index=0;
			var cookieName='tabAddClass'+$this[0].className;

			if($.cookie(cookieName)){
                index=$.cookie(cookieName)*1;
            }else{
				if(opt.init==null){
					index=0;
				}else{
			...
			$tab_li.bind('click',function(){
				...
				$(this).addClass('on');
				for(var i=0;i<len;i++){
					$mColor.removeClass('tab'+(i+1))
				}
				$mColor.addClass('tab'+(index+1));
				$.cookie(cookieName,index,{path:'/',expires:1});
			});
...
$('.colorThema').DB_tabAddClass({init:272});

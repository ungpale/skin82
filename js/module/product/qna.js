            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.comment != undefined) {
                        ...
                        aHtml.push('<a href="#none" class="btnNormal sizeS" onclick="QNA.comment_view('+data.read['no']+');">댓글보기 <em>('+data.read['comment_count']+')</em></a> <a href="#none" class="btnNormal sizeS" onclick="QNA.comment_write(this);"><i class="xi-pen-o"></i> 쓰기</a>');
                        aHtml.push('</div>');
                    }
                    aHtml.push('<span class="gRight">');
                    aHtml.push('<a href="/board/product/modify.html'+ data.read['param_modify'] +'&link_product_no='+iProductNo+'" class="btnNormal sizeS">수정</a>');
                    aHtml.push('</span>');
                    aHtml.push('</div>');
                        ...
                        for (var i=0; data.comment.length > i; i++) {
                            ...
                            if (data.comment[i]['comment_reply_css'] == undefined) {
                                ...
                                if (data.comment[i]['comment_reply_display'] == true) {
                                    ...
                                    aHtml.push('<a href="#none" class="btnNormal" onclick="QNA.comment_reply_view('+data.comment[i]['comment_no']+')">댓글의 댓글 <em>('+data.comment[i]['comment_reply_count']+')</em></a>');
